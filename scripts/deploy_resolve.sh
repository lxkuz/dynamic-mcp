...
resolve_deploy_target() {
  ...
  case "$raw" in
    production|prod)
      ...
      DEPLOY_GIT_BRANCH="${DEPLOY_GIT_BRANCH_PRODUCTION:-main}"
      DEPLOY_GIT_SSH_KEY="${DEPLOY_GIT_SSH_KEY:-${DEPLOY_GIT_SSH_KEY_PRODUCTION:-}}"
      ;;
    staging|stage)
      ...
      DEPLOY_GIT_SSH_KEY="${DEPLOY_GIT_SSH_KEY:-${DEPLOY_GIT_SSH_KEY_STAGING:-}}"
      ;;
    *)
